...
- **Status:** Accepted (amended same-day after author re-read SP-capability-v2 design)
- **Date:** 2026-05-11 · amendment: 2026-05-11 (UCAN re-categorized — see §2.4)
...
|---|---|---|---|---|---|
| 1.A | UCAN capability tokens | ✅ Core vision (Security Layer is the three primitives at L341; concept 4 at L1212-1229; multi-device delegation chain at L1923-1925) | 🔨 SP-capability-v2 (§9.3 amended 2026-05-11; §10 row); spec implementation-ready | Real gate is **sub-agent delegation** (not "multi-tenant per-socket" as originally framed in §9.3): "Agent A delegates read-only Patient X access to sub-agent B." **Triggered** by celia_phr (Hermes orchestrator + N specialised children; celia's flat RBAC forces user to re-pair every child). See SP-capability-v2 §1.2. | **ATD owned; near-term unlock = YES — SP-capability-v2 plan drafted, impl gated only on scheduling.** |
...
- **1.A:** **In flight as SP-capability-v2.** Design landed (`docs/superpowers/specs/2026-05-11-sp-capability-v2-design.md`); implementation plan drafted (`docs/superpowers/plans/2026-05-11-sp-capability-v2.md`); execution gated only on scheduling. UCAN-lite (JWT-shape + Ed25519 + did:key) is additive to the SP-12 string allow-list — SP-12 adopters keep working untouched; clients that supply `Hello.ucan_tokens` get the union. celia_phr is the validation adopter (Hermes orchestrator + delegation chain demo); healthkit_cli benefits passively (no regression on additive change).
